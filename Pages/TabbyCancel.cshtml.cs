...
namespace CRM.Pages
{
    public class TabbyCancelModel : PageModel
    {
        ...
        string res { set; get; }
        public TabbyCancelModel(IRazorPartialToStringRenderer renderer, PerfumeContext context, UserManager<ApplicationUser> userManager, IConfiguration configuration, IEmailSender emailSender, Microsoft.AspNetCore.Hosting.IHostingEnvironment env)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnGet(string payment_id)
        {
            ...
            if (res != null)
            {
                ...
                if (ResStatus.status == "EXPIRED" || ResStatus.is_expired == true || ResStatus.status == "Expired" || ResStatus.status == "expired")
                {
