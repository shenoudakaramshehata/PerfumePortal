...
namespace CRM.Pages
{
    public class SaudiCallBackModel : PageModel
    {
        ...
        string res { set; get; }
        public SaudiCallBackModel(IRazorPartialToStringRenderer renderer,PerfumeContext context, UserManager<ApplicationUser> userManager, IConfiguration configuration, IEmailSender emailSender, Microsoft.AspNetCore.Hosting.IHostingEnvironment env)
        {
            _context = context;
                    ...
                    try
                    {
                        if (fattorahPaymentResult.UserDefinedField != null)
                        {
                            ...
                            if (checkRes)
                            {
                                ...
                                order = await _context.Order.Include(e => e.CustomerN).Include(e => e.CustomerAddress).Include(e => e.OrderStatus).Where(e => e.OrderId == orderId).FirstOrDefaultAsync();
