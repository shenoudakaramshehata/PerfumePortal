...
namespace CRM.Areas.Admin.Pages.Configurations.Operator
{
    public class CheckOutModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPost(string Serial)
        {
            try
            {
                ...
                new Models.Order
                {
                    ...
                    OrderNet = TaxRateValue == 0 ? Math.Round(TotalAmountAfterDiscount + shoppingCost, 2) : Math.Round((double)(((TotalAmountAfterDiscount) * (1 + TaxRateValue)) + shoppingCost), 2),
                ...
                if (paymentId == 1) // My Fatoorah
                {
                    ...
                    var customerAddress = perfumeContext.customerAddresses.Where(c => c.CustomerAddressId == orders.CustomerAddressId).FirstOrDefault();
                    var customerId = perfumeContext.CustomerNs.Where(o => o.CustomerId == customerAddress.CustomerId).FirstOrDefault().CustomerId;
                    var Customer = perfumeContext.CustomerNs.FirstOrDefault(c => c.CustomerId == customerId); if (Customer != null)
                    {
       ...
       .AddParameter("message", "Your order on Mashaer perfumes is now Processing.");
                    ...
                    {
                        return RedirectToPage("/Configurations/Operator/Thankyou", new { orderId = orders.OrderId });
                ...
                else if (paymentId == 2) //Cash On Delivery --> Not Exists on This Applicaiton
                {
                   ...
                   var customerAddress = perfumeContext.customerAddresses.Where(c => c.CustomerAddressId == orders.CustomerAddressId).FirstOrDefault();
                    var customerId = perfumeContext.CustomerNs.Where(o => o.CustomerId == customerAddress.CustomerId).FirstOrDefault().CustomerId;
                    var Customer = perfumeContext.CustomerNs.FirstOrDefault(c => c.CustomerId == customerId);
       ...
       .AddParameter("message", "Your order on Mashaer perfumes is now Processing.");
            ...
            catch (Exception ex)
            {
                ...
                return RedirectToPage("/Configurations/Operator/CheckOut");
            }
            return RedirectToPage("/Configurations/Operator/CheckOut");
